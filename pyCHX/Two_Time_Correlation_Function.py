...
def get_each_frame_ROI_intensity(
    ...
    **kwargs,
    ...
    Get the ROI intensity of each frame
       ...
       Usage:
       imgsum, bad_frame_list = get_each_frame_intensity( data_series,
...
def get_aged_g2_from_g12(g12, age_edge, age_center):
    ...
    namely, calculate the different aged mean of each diag line of g12 to get one-time correlation function
...
def histogram_taus(taus, hisbin=20, plot=True, timeperframe=1):
    ...
    if plot:
        ...
        for key in sorted(his.keys()):
            ...
            ax1.set_ylabel(r"histogram of g2 @ tau", fontsize=15)
            # ax1.set_xscale('log')
...
def get_one_time_from_two_time_old(g12, norms=None, nopr=None):
    ...
    namely, calculate the mean of each diag line of g12 to get one-time correlation function
...
def show_C12(
    ...
    **kwargs,
    ...
    if isinstance(q_ind, int):
        ...
        if num_qr is None:
            raise ValueError(
                "num_qr must be provided when q_ind is a [qz_ind, qr_ind] pair"
            )
        qz_ind, qr_ind = q_ind
        qz_ind, qr_ind = qz_ind - 1, qr_ind - 1
